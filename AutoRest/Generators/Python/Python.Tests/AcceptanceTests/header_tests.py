...
class HeaderTests(unittest.TestCase):
    ...
    def test_headers(self):
        ...
        self.assertEqual(-2, int(raw.response.headers.get("value")))
        ...
        self.assertEqual(105, int(raw.response.headers.get("value")))
        ...
        self.assertTrue(abs(0.07 - float(raw.response.headers.get("value"))) < 0.00001)
        raw = client.header.response_float("negative", raw=True)
        self.assertTrue(abs(-3.0 - float(raw.response.headers.get("value"))) < 0.00001)
        ...
        self.assertEqual(7e120, float(raw.response.headers.get("value")))
        raw = client.header.response_double("negative", raw=True)
        self.assertEqual(-3.0, float(raw.response.headers.get("value")))

        client.header.param_bool("true", True)
        client.header.param_bool("false", False)

        raw = client.header.response_bool("true", raw=True)
        self.assertEqual(True, json.loads(raw.response.headers.get("value")))
        raw = client.header.response_bool("false", raw=True)
        self.assertEqual(False, json.loads(raw.response.headers.get("value")))
