...
Number.prototype.putBigFloat = function (numberBody, options, callback) {
  ...
  // Validate
  try {
    if (numberBody === null || numberBody === undefined || typeof numberBody !== 'double') {
      throw new Error('numberBody cannot be null or undefined and it must be of type double.');
    }
  } catch (error) {
    return callback(error);
  }
