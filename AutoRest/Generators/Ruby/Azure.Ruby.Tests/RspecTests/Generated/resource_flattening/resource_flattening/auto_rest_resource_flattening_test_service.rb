...
module ResourceFlatteningModule
  ...
  class AutoRestResourceFlatteningTestService < MsRestAzure::AzureServiceClient
    ...
    def put_array(resource_array = nil, custom_headers = nil)
      resource_array.each{ |e| e.validate if e.respond_to?(:validate) } unless resource_array.nil?
      # Construct URL
      ...
      unless resource_array.nil?
        serializedArray = []
        resource_array.each do |element|
          unless element.nil?
            element = Resource.serialize_object(element)
          end
          serializedArray.push(element)
        end
        resource_array = serializedArray
      end
      request_content = JSON.generate(resource_array, quirks_mode: true)
          ...
          begin
            ...
            unless parsed_response.nil?
              ...
              parsed_response.each do |element|
                unless element.nil?
                  element = FlattenedProduct.deserialize_object(element)
                end
    ...
    def put_dictionary(resource_dictionary = nil, custom_headers = nil)
      resource_dictionary.each{ |e| e.validate if e.respond_to?(:validate) } unless resource_dictionary.nil?
      # Construct URL
      ...
      # Serialize Request
      request_headers['Content-Type'] = 'application/json; charset=utf-8'
      unless resource_dictionary.nil?
        resource_dictionary.each { |key, valueElement|
          unless valueElement.nil?
            valueElement = FlattenedProduct.serialize_object(valueElement)
          end
          resource_dictionary[key] = valueElement
        }
      end
      request_content = JSON.generate(resource_dictionary, quirks_mode: true)
          ...
          begin
            ...
            unless parsed_response.nil?
              parsed_response.each do |key, valueElement|
                unless valueElement.nil?
                  valueElement = FlattenedProduct.deserialize_object(valueElement)
                end
    ...
    def put_resource_collection(resource_complex_object = nil, custom_headers = nil)
      resource_complex_object.validate unless resource_complex_object.nil?
      # Construct URL
      ...
      # Serialize Request
      request_headers['Content-Type'] = 'application/json; charset=utf-8'
      unless resource_complex_object.nil?
        resource_complex_object = ResourceCollection.serialize_object(resource_complex_object)
      end
      request_content = JSON.generate(resource_complex_object, quirks_mode: true)
      ...
      promise = promise.then do |http_response|
        ...
        if status_code == 200
          begin
            parsed_response = JSON.load(response_content) unless response_content.to_s.empty?
            unless parsed_response.nil?
              parsed_response = ResourceCollection.deserialize_object(parsed_response)
            end
            result.body = parsed_response
          rescue Exception => e
            fail MsRest::DeserializationError.new("Error occured in deserializing the response", e.message, e.backtrace, response_content)
