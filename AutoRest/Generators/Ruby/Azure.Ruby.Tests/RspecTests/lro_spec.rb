...
describe LongRunningOperation do
  before(:all) do
    ...
    @client = AutoRestLongRunningOperationTestService.new(@base_url, TokenCloudCredentials.new(SecureRandom.uuid, SecureRandom.uuid))
  ...
  it 'should wait for succeeded status for create operation' do
    result = @client.lros.put_201_creating_succeeded_200(@product).value!.body
    expect(body.provisioningState).to eq("Succeeded")
  end
  it 'should rise error on "failed" operation result' do
    expect{ @client.lros.put_201_creating_failed_200(@product).value! }.to raise_exception(CloudException, /Long running operation failed/)
  end
  it 'should wait for succeeded status for update operation' do
    result = @client.lros.put_200_updating_succeeded_204(@product).value!.body
    expect(body.provisioningState).to eq("Succeeded")
  end
  it 'should rise error on "canceled" operation result' do
    expect{ @client.lros.put_200_acceptedcanceled_200(@product).value! }.to raise_exception(CloudException, /Long running operation failed/)
  end
  it 'should retry on 500 server responce in POST request' do
    result = @client.lros.post_202_retry_200(@product).value!.response
    expect(result).to be_an_instance_of(Net::HTTPOK)
  end
  it 'should serve success responce on initial PUT request' do
    result = @client.lros.put_200_succeeded(@product).value!.body
    expect(body.provisioningState).to eq("Succeeded")
  end
  it 'should serve success responce on initial request without provision state' do
    result = @client.lros.put_200_succeeded_no_state(@product).value!.body
    expect(body.id).to eq(100)
  end
  it 'should serve 202 on initial responce and status responce without provision state' do
    result = @client.lros.put_200_retry_200(@product).value!.body
    expect(body.id).to eq(100)
  end
