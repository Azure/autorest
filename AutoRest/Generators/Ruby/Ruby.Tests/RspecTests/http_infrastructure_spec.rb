...
describe 'HttpInfrastructure' do
  ...
  context HttpRedirects do
    it 'should send head 300' do
      result = @redirect_client.head300().value!.response
      expect(result).to be_an_instance_of(Net::HTTPMultipleChoices)
    end
    ...
    it 'should send head 301' do
      result = @redirect_client.head301().value!.response
      expect(result).to be_an_instance_of(Net::HTTPMovedPermanently)
    end
    it 'should get 301' do
      result = @redirect_client.get301().value!.response
      expect(result).to be_an_instance_of(Net::HTTPMovedPermanently)
    end
    ...
    it 'should send head 302' do
      result = @redirect_client.head302().value!.response
      expect(result).to be_an_instance_of(Net::HTTPFound)
    end
    ...
    it 'should patch 302' do
      result = @redirect_client.patch302(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPFound)
    end
    it 'should post 303' do
      result = @redirect_client.post303(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPSeeOther)
    end
    it 'should send head 307' do
      result = @redirect_client.head307().value!.response
      expect(result).to be_an_instance_of(Net::HTTPTemporaryRedirect)
    end
    it 'should get 307' do
      result = @redirect_client.get307().value!.response
      expect(result).to be_an_instance_of(Net::HTTPTemporaryRedirect)
    end
    it 'should put 307' do
      result = @redirect_client.put307(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPTemporaryRedirect)
    end
    it 'should patch 307' do
      result = @redirect_client.patch307(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPTemporaryRedirect)
    end
    it 'should post 307' do
      result = @redirect_client.post307(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPTemporaryRedirect)
    end
    it 'should delete 307' do
      result = @redirect_client.delete307(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPTemporaryRedirect)
    end
  end
  context HttpRetry do
    it 'should send head 408' do
      expect{@retry_client.head408().value!}.to raise_exception_with_code(Net::HTTPRequestTimeOut)
    end
    it 'should put 500' do
      expect{@retry_client.put500(true).value!}.to raise_exception_with_code(Net::HTTPInternalServerError)
    end
    it 'should patch 500' do
      expect{@retry_client.patch500(true).value!}.to raise_exception_with_code(Net::HTTPInternalServerError)
    end
    it 'should get 502' do
      expect{@retry_client.get502().value!}.to raise_exception_with_code(Net::HTTPBadGateway)
    end
    it 'should post 503' do
      expect{@retry_client.post503(true).value!}.to raise_exception_with_code(Net::HTTPServiceUnavailable)
    end
    it 'should delete 503' do
      expect{@retry_client.delete503(true).value!}.to raise_exception_with_code(Net::HTTPServiceUnavailable)
    end
    it 'should put 504' do
      expect{@retry_client.put504(true).value!}.to raise_exception_with_code(Net::HTTPGatewayTimeOut)
    end
    it 'should patch 504' do
      expect{@retry_client.patch504(true).value!}.to raise_exception_with_code(Net::HTTPGatewayTimeOut)
    end
  end
  ...
  context HttpSuccess do
    it 'should send head 200' do
      result = @success_client.head200().value!.response
    ...
    it 'should put 200' do
      result = @success_client.put200(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPOK)
    end
    it 'should patch 200' do
      result = @success_client.patch200(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPOK)
    end
    it 'should post 200' do
      result = @success_client.post200(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPOK)
    end
    it 'should delete 200' do
      result = @success_client.delete200(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPOK)
    end
    it 'should put 201' do
      result = @success_client.put201(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPCreated)
    end
    it 'should post 201' do
      result = @success_client.post201(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPCreated)
    end
    it 'should put 202' do
      result = @success_client.put202(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPAccepted)
    end
    it 'should patch 202' do
      result = @success_client.patch202(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPAccepted)
    end
    it 'should post 202' do
      result = @success_client.post202(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPAccepted)
    end
    it 'should delete 202' do
      result = @success_client.delete202(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPAccepted)
    end
    it 'should send head 204' do
      result = @success_client.head204().value!.response
      expect(result).to be_an_instance_of(Net::HTTPNoContent)
    end
    it 'should put 204' do
      result = @success_client.put204(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPNoContent)
    end
    it 'should patch 204' do
      result = @success_client.patch204(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPNoContent)
    end
    it 'should post 204' do
      result = @success_client.post204(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPNoContent)
    end
    it 'should delete 204' do
      result = @success_client.delete204(true).value!.response
      expect(result).to be_an_instance_of(Net::HTTPNoContent)
    end
  end
  context MultipleResponses do
    ...
    it 'should get default 200 valid' do
      result = @multiple_resp_client.get_default_model_a200valid().value!.response
      expect(result).to be_an_instance_of(Net::HTTPOK)
    end
