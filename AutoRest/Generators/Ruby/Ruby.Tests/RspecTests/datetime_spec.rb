...
describe Date do
  ...
  it 'should get invalid' do
    expect{@date_client.get_invalid().value!}.to raise_error(ArgumentError)
  end
  it 'should get overflow' do
    result = @date_client.get_overflow().value!
    expect(result.response).to be_an_instance_of(Net::HTTPOK)
    expect(result.body).to eq(DateTime.new(9999,12,31,23,59,59,'+15'))
  end
  it 'should get utc lowercase max date time' do
    result = @date_client.get_utc_lowercase_max_date_time().value!
    expect(result.response).to be_an_instance_of(Net::HTTPOK)
    expect(result.body).to eq(DateTime.new(9999,12,31,23,59,59,'Z'))
  end
  ...
  it 'should get local negative offset lowercase max date time' do
    ...
    expect(result.response).to be_an_instance_of(Net::HTTPOK)
    expect(result.body).to eq(DateTime.new(9999,12,31,23,59,59,'-14:00'))
  end
  ...
  it 'should get underflow' do
    expect{@date_client.get_underflow().value!}.to raise_error(ArgumentError)
  end
  it 'should put utc max date' do
    pending("Test pended because DateTime sends local time, i.e. '+00:00', not 'Z'")
    result = @date_client.put_utc_max_date_time(DateTime.new(9999,12,31,23,59,59.9999999, 'Z')).value!
    expect(result.response).to be_an_instance_of(Net::HTTPOK)
  end
  it 'should put utc min date' do
    pending("Test pended because DateTime sends local time, i.e. '+00:00', not 'Z'")
    result = @date_client.put_utc_min_date_time(DateTime.new(0001,01,01,00,00,00,'Z')).value!
    expect(result.response).to be_an_instance_of(Net::HTTPOK)
  end
