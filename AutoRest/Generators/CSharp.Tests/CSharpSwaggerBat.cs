...
using Fixtures.SwaggerBatBodyArray;
using Fixtures.SwaggerBatBodyArray.Models;
using Fixtures.SwaggerBatBodyBoolean;
using Fixtures.SwaggerBatBodyByte;
using Fixtures.SwaggerBatBodyDate;
...
using Fixtures.SwaggerBatBodyString.Models;
using Fixtures.SwaggerBatBodyComplex;
using Fixtures.SwaggerBatBodyComplex.Models;
using Fixtures.SwaggerBatHttp;
using Fixtures.SwaggerBatHttp.Models;
using Fixtures.SwaggerBatReport;
using Fixtures.SwaggerBatUrl;
using Fixtures.SwaggerBatUrl.Models;
    ...
    public class CSharpSwaggerBat : IClassFixture<ServiceController>
    {
        ...
        public void BoolTests()
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\body-boolean.json", @"Expected\SwaggerBat\BodyBoolean.cs");
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\body-integer.json", @"Expected\SwaggerBat\BodyInteger.Cs");
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\body-number.json", @"Expected\SwaggerBat\BodyNumber.Cs");
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\body-string.json", @"Expected\SwaggerBat\BodyString.cs");
            var client = new AutoRestSwaggerBATService(Fixture.Uri);
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\body-byte.json", @"Expected\SwaggerBat\BodyByte.Cs");
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\body-date.json", @"Expected\SwaggerBat\BodyDate.Cs");
            var client = new AutoRestDateTestService(Fixture.Uri);
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\body-datetime.json", @"Expected\SwaggerBat\BodyDateTime.Cs");
            var client = new AutoRestDateTimeTestService(Fixture.Uri);
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\body-array.json", @"Expected\SwaggerBat\BodyArray.Cs");
            var client =
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\body-dictionary.json", @"Expected\SwaggerBat\BodyDictionary.Cs");
            var client =
            ...
            foreach (var key in dictionaryDictionary.Keys)
            {
                ...
                Assert.Equal(dictionaryDictionary[key], dictionaryResult[key], new DictionaryEqualityComparer<string>());
            }
            // GET dictionary/null
            Assert.Null(client.Dictionary.GetComplexNull());
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\body-complex.json", @"Expected\SwaggerBat\BodyComplex.Cs");
            var client = new AutoRestComplexTestService(Fixture.Uri);
            ...
            var polymorphismRequest = new Salmon
            {
                ...
                {
                    new Shark {Age = 6, Length = 20, Species = "predator", Birthday = new DateTime(2012, 1, 5, 1, 0, 0, DateTimeKind.Utc)},
                    new Sawshark {Age = 105, Length = 10, Species = "dangerous", Birthday = new DateTime(1900, 1, 5, 1, 0, 0, DateTimeKind.Utc), Picture = new byte[] {255, 255, 255, 255, 254}}
                }
            };
            ...
            var badRequest = new Salmon
            {
                ...
                Siblings = new List<Fish>
                {
                    ...
                    new Sawshark {Age = 105, Length = 10, Species = "dangerous",  Picture = new byte[] {255, 255, 255, 255, 254}}
                }
            };
            var missingRequired =
                ...
                Siblings = new List<Fish>
                {
                    new Shark
                    {
                        ...
                        Siblings = new List<Fish>
                        {
                            ...
                            new Sawshark {Age = 105, Length = 10, Species = "dangerous", Siblings = new List<Fish>(), Birthday = new DateTime(1900, 1, 5, 1, 0, 0, DateTimeKind.Utc), Picture = new byte[] {255, 255, 255, 255, 254}}
                        }, 
                        Birthday = new DateTime(2012, 1, 5, 1, 0, 0, DateTimeKind.Utc)
                    },
                    new Sawshark {Age = 105, Length = 10, Species = "dangerous", Siblings = new List<Fish>(), Birthday = new DateTime(1900, 1, 5, 1, 0, 0, DateTimeKind.Utc), Picture = new byte[] {255, 255, 255, 255, 254}}
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\url.json", @"Expected\SwaggerBat\Url.Cs");
            var client = new AutoRestUrlTestService(Fixture.Uri);
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\header.json", @"Expected\SwaggerBat\Header.Cs");
            var client = new AutoRestSwaggerBATHeaderService(Fixture.Uri);
            ...
            Assert.Equal("The quick brown fox jumps over the lazy dog", response.Response.Headers.GetValues("value").FirstOrDefault());
        ...
        {
             SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\httpInfrastructure.json", @"Expected\SwaggerBat\Http.Cs");
            ...
            EnsureThrowsWithStatusCode(HttpStatusCode.Created, () => client.MultipleResponses.Get200Model204NoModelDefaultError201Invalid());
            EnsureThrowsWithStatusCode(HttpStatusCode.Accepted, () => client.MultipleResponses.Get200Model204NoModelDefaultError202None());
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\required-optional.json", @"Expected\SwaggerBat\RequiredOptional.Cs");
        ...
        public void RequiredOptionalTests()
        {
            ...
            Assert.Equal(HttpStatusCode.OK,
                client.ImplicitModel.GetOptionalGlobalQueryWithOperationResponseAsync(null).Result.Response.StatusCode);

            Assert.Equal(HttpStatusCode.OK,
                client.ExplicitModel.PostOptionalIntegerParameterWithOperationResponseAsync(null).Result.Response.StatusCode);
            Assert.Equal(HttpStatusCode.OK,
            ...
            Assert.Equal(HttpStatusCode.OK,
                client.ExplicitModel.PostOptionalIntegerHeaderWithOperationResponseAsync(null).Result.Response.StatusCode);
            Assert.Equal(HttpStatusCode.OK,
                client.ExplicitModel.PostOptionalStringParameterWithOperationResponseAsync(null).Result.Response.StatusCode);
            Assert.Equal(HttpStatusCode.OK,
                ...
                .Result.Response.StatusCode);
            Assert.Equal(HttpStatusCode.OK,
                client.ExplicitModel.PostOptionalStringHeaderWithOperationResponseAsync(null).Result.Response.StatusCode);
            Assert.Equal(HttpStatusCode.OK,
                client.ExplicitModel.PostOptionalClassParameterWithOperationResponseAsync(null).Result.Response.StatusCode);
            Assert.Equal(HttpStatusCode.OK,
                ...
                .Result.Response.StatusCode);
            Assert.Equal(HttpStatusCode.OK,
                client.ExplicitModel.PostOptionalArrayParameterWithOperationResponseAsync(null).Result.Response.StatusCode);
            Assert.Equal(HttpStatusCode.OK,
                ...
                .Result.Response.StatusCode);
            Assert.Equal(HttpStatusCode.OK,
                client.ExplicitModel.PostOptionalArrayHeaderWithOperationResponseAsync(null).Result.Response.StatusCode);
        }
        ...
        {
            SwaggerSpecHelper.RunTests<CSharpCodeGenerator>(
                @"Swagger\SwaggerBat\report.json", @"Expected\SwaggerBat\Report.Cs");
            var client =
