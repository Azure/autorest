# Copyright (c) Microsoft Corporation. All rights reserved.
# Licensed under the MIT License. See License.txt in the project root for license information.

require 'client_runtime'

require 'client_runtime_azure/version'

require 'client_runtime_azure/resource.rb'
require 'client_runtime_azure/cloud_error.rb'

require 'client_runtime_azure/async_operation_status.rb'
require 'client_runtime_azure/polling_state.rb'

require 'client_runtime_azure/subscription_cloud_credentials.rb'
require 'client_runtime_azure/token_cloud_credentials.rb'

require 'client_runtime_azure/azure_service_client.rb'

module ClientRuntimeAzure; end
