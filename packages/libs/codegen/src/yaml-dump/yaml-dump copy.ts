...
function writeNode(state, level, object, block, compact, iskey, isblockseq) {
  ...
  let duplicateIndex = state.usedDuplicates.get(object);
  if (duplicate && duplicateIndex !== undefined) {
    state.dump = "*ref_" + duplicateIndex;
  } else {
    if (objectOrArray && duplicate && duplicateIndex === undefined) {
      duplicateIndex = state.indexCounter++;
      state.usedDuplicates.set(object);
    }
