...
async function composeSwaggers(
  ...
  try {
    ...
    if (candidateTitles.length > 1) {
      throw new Error(
        `The 'title' across provided OpenAPI definitions has to match. Found: ${candidateTitles
          .map((x) => `'${x}'`)
    ...
    for (let i = 0; i < inputSwaggers.length; ++i) {
      ...
      const parameters = methods
        .SelectMany((method: any) => getArrayValues<any>(getProperty<any, any>(method, "parameters")))
        .Concat(allPaths.SelectMany((path: any) => getArrayValues<any>(getProperty<any, any>(path, "parameters"))));

      // inline api-version params
      if (!uniqueVersion) {
        const clientParams = swagger.components.parameters || {};
        const apiVersionClientParamName = Object.getOwnPropertyNames(clientParams).filter(
          (n) => clientParams[n].name === "api-version",
        )[0];
        ...
        if (apiVersionClientParam) {
          ...
          const apiVersionParameters = parameters.Where((p: any) =>
            p.obj.$ref.endsWith(`#/components/parameters/${apiVersionClientParamName}`),
          );
          for (const apiVersionParameter of apiVersionParameters) {
            ...
            mapping.push(
              ...CreateAssignmentMapping(
                apiVersionClientParam,
                inputSwagger.key,
                ["parameters", apiVersionClientParamName],
                apiVersionParameter.path,
            ...
            mapping.push({
              name: "inlining api-version",
              source: inputSwagger.key,
...
export function createComposerPlugin(): PipelinePlugin {
  return async (config, input, sink) => {
    ...
    const result = await sink.WriteObject(
      swagger.Description,
      await refCleaner.getOutput(),
      swagger.identity,
      swagger.artifactType,
      await refCleaner.getSourceMappings(),
      [swagger],
    );
