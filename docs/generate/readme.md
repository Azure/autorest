...
|`--head-as-boolean`|With this flag, HEAD calls to non-existent resources (404) will not raise an error. Instead, if the resource exists, we return `true`, else `false`. Forced to be `true` if `--azure-arm` is set, otherwise defaults to `false`.| x | .NET default is always false | no head-as-boolean | x | x | x |
|`--client-side-validation`|Whether you want the SDK to perform validation on inputs and outputs, based on swagger information. Recommended to be `false` for track 2 code, since we want the network to validate instead. Defaults to `false`.| x | x | calls it `--client-side-validations` | x | x | x |
|`--title=NAME`|Override the service client's name listed in the swagger under `title`.| x | x | x | x | x | x |
|`--description=DESCRIPTION`|Override the service client's description listed in the swagger under the top level `description`.| x | x | x | x | x | x |
|`--package-name=NAME`|The name of your package. This i the name your package will be published under.| x | | |x | | |
|`--no-async`|Async code is not generated if this flag is added to the command line. Defaults to `false`.| x |  |  | x | x | x |
