    ...
    public class ExtensionsTests
    {
        ...
        public void TestParameterLocationExtension()
        {
            using (NewContext)
            {
                ...
                Assert.True(
                    clientModel.Methods[0].Parameters.Any(p => p.Name == "resourceGroupName" && !p.IsClientProperty));
                ...
                Assert.True(
                    clientModel.Methods[1].Parameters.Any(p => p.Name == "resourceGroupName" && !p.IsClientProperty));
        ...
        public void TestClientModelWithPayloadFlatteningViaXMSClientFlatten()
        {
            using (NewContext)
            {
                ...
                Assert.True(
                    simpleProduct.Properties.Any(p => p.SerializedName == "details.max_product_image.@odata\\\\.value"
                                                      && p.Name == "@odata.value"));
                ...
                Assert.True(conflictedProduct.Properties.Any(p => p.SerializedName == "details.max_product_display_name"
                                                                  &&
                                                                  p.Name ==
                                                                  "ConflictedProductProperties_max_product_display_name"));
                Assert.True(
                    conflictedProduct.Properties.Any(p => p.SerializedName == "simpleDetails.max_product_display_name"
                                                          &&
                ...
                Assert.True(
                    error.Properties.First(p => p.SerializedName == "parentError" && p.Name == "parentError").ModelType ==
                    error);
            }
        }
