...
async function compose(config: ConfigurationView, input: DataSource, sink: DataSink) {
  ...
  return new QuickDataSource([await sink.WriteObject('composed oai3 doc...', await composer.getOutput(), [].concat.apply([], <any>inputs.map(each => each.identity)), 'merged-oai3', await composer.getSourceMappings())], input.skip);
}
