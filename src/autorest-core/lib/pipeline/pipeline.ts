...
export async function RunPipeline(config: ConfigurationView, fileSystem: IFileSystem): Promise<void> {
  ...
  if (!config.DisableValidation) {
    if (config.GetEntry("azure-validator-individual")) {
      barrier.Await(RunPlugin([...config.GetPluginViews("azure-validator-individual")][0], "azure-validator", scopeLoadedSwaggersTransformed));
    }
  }
  ...
  if (!config.DisableValidation) {
    ...
    if (config.GetEntry("azure-validator-composed")) {
