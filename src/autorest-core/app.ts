  ...
  for (const arg of autorestArgs) {
    ...
    if (match === null) {
      if (result.configFileOrFolder) {
        throw new Error(
          `Found multiple configuration file arguments: '${
            result.configFileOrFolder
          }', '${arg}'`
        );
      }
...
function getCommandLineArguments(autorestArgs: string[]): CommandLineArgs {
  ...
  return parseArgs([...autorestArgs, ...more]);
}
...
async function currentMain(autorestArgs: string[]): Promise<number> {
  ...
  const args: CommandLineArgs = getCommandLineArguments(autorestArgs);
  ...
  if (config.HelpRequested) {
    ...
    const helpArtifacts = artifacts.sort(
      (a, b) =>
        a.uri === b.uri
          ? a.content > b.content
            ? 1
    ...
    for (const helpArtifact of helpArtifacts) {
      ...
      for (const settingHelp of help.settings) {
        ...
        const typePart = settingHelp.type ? `=<${settingHelp.type}>` : ` `;
 ...
 * Entry point
 */
async function mainImplementation(): Promise<number> {
  let autorestArgs: string[] = [];
  let exitCode: number = 0;

  try {
    autorestArgs = process.argv.slice(2);
    exitCode = isLegacy(autorestArgs)
...
async function main() {
  ...
  try {
    exitCode = await mainImplementation();
  } catch {
    exitCode = 102;
  } finally {
    try {
      ...
      // TODO: do something when something is catched.
    } finally {
