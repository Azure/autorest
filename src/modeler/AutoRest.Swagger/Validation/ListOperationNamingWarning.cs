    ...
    public class ListOperationNamingWarning : TypedRule<Dictionary<string,Dictionary<string, Operation>>>
    {
        ...
        public override IEnumerable<ValidationMessage> GetValidationMessages(Dictionary<string,Dictionary<string, Operation>> entity, RuleContext context)
        {
            ...
            foreach (var opPair in listOperations)
            {
                ...
                {
                    yield return new ValidationMessage(new FileObjectPath(context.File, context.Path.AppendProperty(violatingPath).AppendProperty(opPair.Key).AppendProperty(opPair.Value.OperationId)), this, opPair.Value.OperationId, XmsPageableViolation);
                }
                else if (ValidationUtilities.IsArrayTypeResponseOperation(opPair.Value, serviceDefinition))
                {
                    yield return new ValidationMessage(new FileObjectPath(context.File, context.Path.AppendProperty(violatingPath).AppendProperty(opPair.Key).AppendProperty(opPair.Value.OperationId)), this, opPair.Value.OperationId, ArrayTypeViolation);
                }
            }
        }
